...
class RadioForm(forms.ModelForm):
    ...
    def clean_TEI(self):
	    ...
	    if not raw_input.isdigit():
	        raise forms.ValidationError("TEI must contain only digits.")

	    if len(raw_input) not in (14, 15):
	        raise forms.ValidationError("TEI must be 14 or 15 digits long.")

	    if len(raw_input) == 15:
	        # if length is 15, last digit must be zero and removed
	        if not raw_input.endswith('0'):
	            raise forms.ValidationError("If 15 digits, last digit must be zero.")
	    ...
	    if not TEIRange.objects.filter(min_tei__lte=tei_int, max_tei__gte=tei_int).exists():
	        raise forms.ValidationError("TEI is not within known TEI ranges.")
